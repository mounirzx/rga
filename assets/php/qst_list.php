...
try {
    ...
    $sql = 'SELECT id_questionnaire,user,nom_exploitant,prenom_exploitant,nom_exploitation,exploitant_cle_unique,exploi_superficie_hec,wilaya_name_ascii,commune_name_ascii,nom_recensseur,prenom_recenseur,message_coherence_stat_jur,message_coherence_util_sol,coherence_stat_jur,coherence_util_sol,etat FROM `questionnaire` left join recenseur on questionnaire.user = recenseur.id_user  left join controleur on  recenseur.controleur  = controleur.id_user  left join communes on recenseur.commune = communes.commune_code  left join coherence_superficie on  questionnaire.id_questionnaire = coherence_superficie.id_quest  where communes.wilaya_code = '.$wilaya_code.' ' ;
