...
try {
    ...
    $fieldsQuestionnaire = array_diff($fieldsQuestionnaire, ['origine_des_terres', 'status_juridique', 'superfecie_sj', 'superficie_are','superfecie_sj_are']);
    ...
    // foreach ($formDataArrayStatut as $formData) {
    //     if (!empty($formData['origine_des_terres']) && !empty($formData['status_juridique']) && !empty($formData['superfecie_sj']) && !empty($formData['superfecie_sj_are'])) {
    //         $cle_status_juridique = substr($lastInsertId . "-" . $formData['status_juridique'], 0, 8);
    
    //         // Debugging output
    //         $debugData = "Debug: " . print_r($cle_status_juridique, true);
    //         file_put_contents(__DIR__ . '/logfile.log', $debugData, FILE_APPEND);
...
//            $cle_status_juridique=$lastInsertId."-".$formData->origine_des_terres."-".$formData->status_juridique;
//         //    ob_start();
//         //    var_dump($formDataArrayStatut);
//         //    $logData = ob_get_clean();
...
//  $reqStatusJuridique = $bdd->prepare("INSERT INTO `status_juridique` (`id_questionnaire`, `cle_status_juridique`,`origine_des_terres`, `status_juridique`, `superfecie_sj`, `superfecie_sj_are`) VALUES (:id_questionnaire, :cle_status_juridique, :origine_des_terres, :status_juridique, :superfecie_sj, :superfecie_sj_are)");
//             $reqStatusJuridique->execute([
//                 'id_questionnaire' => $lastInsertId,
//                 'cle_status_juridique'=> $cle_status_juridique,
//                 'origine_des_terres' => $formData->origine_des_terres,
