...
<span class="input-group-text" id="basic-addon3"> الرقم <br>
   Numéro</span>
   <input  <?= $disabled ?>   class="form-control bneder" id="num_qst" name="code_validation" disabled style="color:#e9ecef">
    <!-- <input  <?= $disabled ?>   class="form-control bneder" id="num_qst11" name="code_validation11" disabled> -->
</div> 
...
<script>
                       ...
                       <input hidden disabled readonly  id="id_status_juridique" name="id_status_juridique"  class="form-control superficie_are coherence_surface_total-surface_are  surface_total_error_are statut_juridique_s"  >
                        ...
                        <div class="col-1">
                            ...
                            <div class="d-grid gap-2">
                                        <button  <?= ($_SESSION['role'] !== "recenseur" || $etat == "Approuvés") ? '' : 'id="addFormU"' ?> style="width: 28px;top: 121px; width: 50px;height: 34px;position: absolute;left: 698px;z-index: 500" class="btn btn-primary btn-sm" type="button" >+</button>
...
function generateCombinationSj(row) {
    return row.find('[id^="status_juridique"]').val() + '-' +
           row.find('[id^="origine_des_terres"]').val() 
         
}

// Function to check and manage unique combinations
function manageCombinations(row) {
    var key = generateCombinationSj(row);
    if (combinations[key]) {
        Swal.fire({
            title: 'Attention!',
...
<div class="card" style="font-size: 12px;">
   ...
   <div id="formContainer2">
        <div class="row code_culture_s" style="margin-bottom: 10px;">
          <div class="col-6">
            <div class="input-group input-group-sm">
              ...
              <div class="input-group input-group-sm">
                ...
                <input hidden disabled readonly id="id" name="id"  num class="form-control class_intercalaire code_culture_s"  value="">
              </div>
              </div>
          ...
          <div class="col-1">
            <div class="d-grid gap-2">
            <button   <?= ($_SESSION['role'] !== "recenseur" || $etat == "Approuvés") ? '' : 'id="addForm2U"' ?> style="width: 50px;top: 107px;position: absolute;height: 34px;left: 698px;z-index: 500" class="btn btn-primary btn-sm" type="button" >+</button>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="row code_materiel_s" style="margin-bottom: 10px;">
    ...
    <div class="col">
      <div class="input-group input-group-sm">
        ...
        <input hidden disabled readonly id="id_materiel_agricol" name="id_materiel_agricol"  class="form-control code_materiel_s" value="">
      </div>
      <div class="col-2"></div>
    </div>
    <div class="col-1">
      <div class="d-grid gap-2">
        <button   <?= ($_SESSION['role'] !== "recenseur" || $etat == "Approuvés") ? '' : 'id="addForm3U"' ?> style="width: 50px;top: 110px;position: absolute;height: 35px;left: 698px;z-index: 500" class="btn btn-primary btn-sm" type="button">+</button>
      </div>
    </div>
  </div>
</div>
...
// Function to generate a unique combination based on row inputs
function generateCombinationMa(row) {
...
// Function to check and manage unique combinations
function manageCombinations(row) {
    var key = generateCombinationMa(row);
    if (combinations[key]) {
        Swal.fire({
            title: 'Attention!',
            text: 'Cette combinaison a déjà été sélectionnée. Veuillez en choisir une autre.',
...
// Handle the addition of new rows
  ...
  $('#addForm3U').click(function() {
    ...
    var removeButton = $('<button>').text('-').addClass('btn btn-danger btn-sm').click(function() {
      delete combinations[generateCombinationMa($(this).closest('.row'))];
...
data.materiel_agricole.forEach(function(item) {
           //console.log(data[i].code_materiel)
            inputs += '<div class="row code_materiel_s" style="margin-bottom: 10px;">';
...
inputs += '</select>';
   ...
   inputs += '</select>';
   inputs += '<input hidden disabled readonly id="in114" value="' + item.id_materiel_agricol + '" name="id_materiel_agricol"  class="form-control" value="">';
   inputs += '</div>';
   inputs += '</div>';

    inputs += '<div class="col-1">';
    inputs += '<div class="d-grid gap-2"><button <?= ($_SESSION['role'] !== "recenseur" || $etat == "Approuvés") ? 'disabled' : '' ?> style=" opacity: 1;width:41px" type="button" class="btn btn-danger btn-sm delete-btn delete_materiel_agricole" data-code-materiel="' + item.id_materiel_agricol + '" id="delete-' + item.code_materiel_nombre + '-' + item.ee_mode_mobilisation_materiel + '" >-</button></div>';
    inputs += '</div>';
    inputs += '</div>';
});
...
$(document).on('click', '.delete_materiel_agricole', function() {
    ...
    var id_materiel_agricol = $(this).data('code-materiel');
    var buttonId = $(this).attr('id');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: 'assets/php/delete_materiel_agricole.php',
                type: 'POST',
                data: { id_materiel_agricol: id_materiel_agricol },
                success: function(response) {
                    // Remove the corresponding row visually from the table
                    $('#' + buttonId).closest('.row').remove();
 ...
 var elements = document.getElementsByClassName("surface");
    for (var i = 0; i < elements.length; i++) {
      ...
      //  console.log(surfaces_improductives_1)
        var superficie_agricole_totale_sat_1 = pacages_et_parcours_1 + surfaces_improductives_1 + superficie_agricole_utile_sau_3 + superficie_agricole_utile_sau_1;
        document.getElementsByName("superficie_agricole_totale_sat_1")[0].value = (superficie_agricole_totale_sat_1);
        ...
        document.getElementsByName("surface_totale_st_1")[0].value = (surface_totale_st_1);
...
'</select>'+
        ...
        '<input hidden disabled readonly id="id_status_juridique"  name="id_status_juridique"  style="max-width: 50px;"  class="form-control statut_juridique_s"  " value="' + (item.id_status_juridique || '') + '">' +
        '</div>' +
        '</div>' +
        '<div class="col">' +
        '<div class="d-grid gap-2">'+
        '<button  <?= ($_SESSION['role'] !== "recenseur" || $etat == "Approuvés") ? 'disabled' : '' ?> style="  opacity: 1;position: relative; right: 0px; top: 0px; z-index: 500" class="btn btn-danger btn-sm disable-44-45-46" type="button" id="delete-' + item.origine_des_terres + '" data-code-origine_des_terres="' + item.cle_status_juridique + '" >-</button>' +
        '</div>' +
...
$('#exploitant').on('change', function() {
      var exploitantValue = $(this).val();
     if(exploitantValue === "2" || exploitantValue === "1"){
      
      listOrigineTerre ["5"]='<option selected="" disabled BoldText>-</option><option value="12">12 - Droit d’usage des forêts حق الانتفاع في استخدام الغابات للملكية العمومية</option><option value="22" BoldText>22 - Inconnu غير معروف</option>'
...
if (selectedValue == "5") {
  $('#status_juridique_' + idPart).empty();
  $('#status_juridique_' + idPart).append('<option selected="" disabled BoldText>-</option><option value="12">12 - Droit d’usage des forêts حق الانتفاع في استخدام الغابات للملكية العمومية</option><option value="22" BoldText>22 - Inconnu غير معروف</option>');
}
    })
